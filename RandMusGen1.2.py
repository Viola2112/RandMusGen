# Random Music Generator 1.2

# Improvements from RandMusGen 1.0:
    # Added a new style called "Returning First"

# Improvements from RandMusGen 1.1:
    # Put on limitations so the music never strays
    # more than one octave from Note(0)
...
class Note:
    ...
    def variation(self,seed):
        # Limitations put on so no more than
        # 1 octave from the 0 note
        if (seed < 0.5 and self.v >= -7) or (self.v > 7):
...
def expandMeasure(initNote,seed,style="Original"):
    noteList = []
    currNote = initNote
    firstNote = initNote
    noteList.append(currNote)
